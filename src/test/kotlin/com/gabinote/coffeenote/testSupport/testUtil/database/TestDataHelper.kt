...
class TestDataHelper(
    ...
    fun enablePreImages(collectionName: String) {

        if (!mongoTemplate.collectionExists(collectionName)) {
            mongoTemplate.createCollection(collectionName)
        }

        // db.runCommand({ collMod: "notes", changeStreamPreAndPostImages: { enabled: true } })
        val command = Document("collMod", collectionName)
            .append("changeStreamPreAndPostImages", Document("enabled", true))

        val result = mongoTemplate.executeCommand(command)

        logger.debug { "Enabled pre-images for collection '$collectionName': $result" }
    }
