...
import com.gabinote.coffeenote.testSupport.testDocs.user.UserDocsSchema
import com.gabinote.coffeenote.testSupport.testTemplate.WebMvcTestTemplate
import com.gabinote.coffeenote.testSupport.testUtil.data.user.UserTestDataHelper.createTestUserFullResControllerDto
import com.gabinote.coffeenote.testSupport.testUtil.data.user.UserTestDataHelper.createTestUserMinimalResControllerDto
                ...
                describe("nickname 검증 테스트") {
                    ...
                    describe("성공케이스") {
                        context("nickname이 정확히 2자면") {
                            ...
                            beforeTest {
                                every {
                                    userMapper.toRegisterReqServiceDto(
                                        request,
                                        requestorUid
                                    )
                                } returns serviceReqDto
                                every { userService.createUser(serviceReqDto) } returns serviceDto
                                every { userMapper.toFullResControllerDto(serviceDto) } returns expected
                            }
